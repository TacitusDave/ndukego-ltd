...
| 27 | 2026-09-23 | Reliability + UX overhaul: storage LRU cache + negative caching + ETag/304 (fixes slow property-card images and DB hammering); fixed delete-before-update ordering bugs in estate site-plan and poster upload (orphaned image refs); JWT validation falls back to token claims on transient DB errors (no more silent logouts during Neon cold starts); multer size limits on all memory-storage upload endpoints; health endpoint now reports DB + storage subsystem status; admin/web error boundaries + not-found pages (retry UI instead of red dumps); cookie-safe session refresh (prerender crashes fixed); admin image previews unified on mediaUrl helper; site-plan no longer renders built-in demo fallback on web; EstateCard shows site-plan chip (home featured + real-estate page); hero redesigned (60/40 split, swipe-card transition at 0.7s, 4.5s tempo, vertical prev/next buttons on divider, centered mobile text); restrained motion system (reveal.ts 14px rise, static process section, prefers-reduced-motion support); auto-fading scrollbar |
| 28 | 2026-09-24 | "How It Works" section redesigned as a laptop-only expanding-card accordion — the active step grows into a giant card (big number, title, description, photo below the text) while the other three collapse into slim columns (number, vertical title, plus affordance); clicking a collapsed card expands it and closes the previous one, step 01 open by default; 0.7s ease transition, keyboard-accessible (role=button + Enter/Space), placeholder Background images from /public until the real ones are dropped; tablet/mobile keeps the original static 2-col grid; section copy rewritten to be service-neutral ("One process, every service") so the How-It-Works journey spans all four services — real estate, LPO financing, investment financing, consultancy — instead of reading as a property-purchase-only flow; fixed hero slideshow hydration mismatch — initial slide sequence is now a deterministic [1..6] constant (Math.random() shuffle in the useState initializer produced different server/client orders), reshuffled client-side after mount with Background1 kept as slide 0 so the displayed image never jumps |
| 29 | 2026-09-24 | Posters → Gallery migration — Prisma model Poster→GalleryItem (new table gallery_items; old poster data recovered from upload_files blobs via packages/database/scripts/recover-gallery-from-posters.mjs), API PosterModule→GalleryModule (CRUD + toggle + public endpoint; IMAGE/VIDEO/COLLAGE types, 150MB video uploads with optional poster frames, header-based image dimension detection, blob cleanup on replace/delete); public /gallery page with grouped adaptive 1–5 column grid, media-kind filter chips, full lightbox (zoom to 1:1, download, share, copy link, keyboard nav, video playback with poster frames); admin /gallery manager (image/video upload + edit dialog, poster-frame upload, auto dimension detection, show/hide toggle, group labels); estates pages folded into the Real Estate service page (EstateListing component embeds the catalogue) and estate detail moved to /services/real-estate/estates/[id] with all cards/footer/CTA links updated; navbar "Estates" replaced with "Gallery"; footer rebuilt (Explore column, RC 1164064 badge, live-data estate column removed, footer now renders without API); footer hidden on /contact; fixes in this pass: recovery script was missing the required collageImages field (would crash with Prisma P2022) and contact page had invalid pb-s24 Tailwind class (→ pb-24); verified api/web/admin typechecks + Prisma validate + production builds of web (24 routes) and admin; live smoke-tested /gallery/public API and web /gallery + /services/real-estate (200s) |
